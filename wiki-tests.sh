...
if [ "$4" == "update" ]; then
    ...
    wget -O $TMPHTML -q https://wiki.apertium.org/wiki/Macedonian_and_English/$TESTTYPE
    if [[ -s $TMPHTML ]]; then mv $TMPHTML $HTML;
    else rm $TMPHTML; echo "Couldn't fetch https://wiki.apertium.org/wiki/Macedonian_and_English/$TESTTYPE"; fi
fi
echo "..."
